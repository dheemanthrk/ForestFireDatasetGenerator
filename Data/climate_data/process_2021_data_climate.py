...
climate_df = climate_df.drop(columns=['number'], errors='ignore')  # Drop 'number' column if present

# Renaming columns to the desired format
...
grid_gdf = grid_gdf.to_crs("EPSG:4326")  # Reproject to latitude/longitude
grid_gdf["latitude"] = grid_gdf.geometry.y
grid_gdf["longitude"] = grid_gdf.geometry.x
grid_gdf = grid_gdf.set_geometry("geometry")  # Reset original geometry as active geometry
...
fire_data['REP_DATE'] = pd.to_datetime(fire_data['REP_DATE'], errors='coerce')  # Parse dates
...
# Reorder columns to place latitude and longitude at the end
combined_data = combined_data[['grid_id', 'Date', 'avg_temperature', 'total_precipitation', 'avg_wind_speed', 'avg_humidity', 'avg_solar_radiation', 'avg_soil_moisture', 'latitude', 'longitude', 'fire_occurred']]

# Save to CSV
output_path = 'combined_climate_fire_data_with_latlong.csv'
